...
public static class PlanetOrderSceneBuilder
{
    private const string SceneFolder = "Assets/Game/Scenes/PlanetOrderGame";
    private const string ScenePath = "Assets/Game/Scenes/PlanetOrderGame/PlanetOrderGame.unity";
    ...
    [MenuItem("Tools/PlanetOrder/Build Scene")]
    public static void BuildAll()
    {
        if (EditorApplication.isPlaying) { Debug.LogError("PlanetOrderSceneBuilder: Cannot build in Play mode."); return; }
        EnsureFolder("Assets/Game"); EnsureFolder("Assets/Game/Scenes"); EnsureFolder(SceneFolder);
        BuildScene();
        AddSceneToBuildSettings(ScenePath);
        AssetDatabase.SaveAssets(); AssetDatabase.Refresh();
        Debug.Log("PlanetOrderSceneBuilder: finished.");
    }
    ...
    private static void BuildScene()
    {
        ...
        GameObject root = new GameObject("PlanetOrderRoot");
        PlanetOrderController ctrl = root.AddComponent<PlanetOrderController>();
        PlanetOrderView view = root.AddComponent<PlanetOrderView>();

        // === BG, Team bars, Timer, Score bars, Nav buttons (copy from AddUp) ===
        ...
        // === PLAYER PANELS (split screen, 5 boxes each max) ===
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            { imp.textureType = TextureImporterType.Sprite; imp.spriteImportMode = SpriteImportMode.Single; imp.mipmapEnabled = false; imp.SaveAndReimport(); }
    ...
    /// <summary>
    /// Create 5 number boxes for a player (left or right half of screen).
    /// Extra boxes start hidden — View activates them as difficulty increases.
    /// </summary>
        ...
        Button[] btns, Text[] txts, Image[] imgs)
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            // Default position — will be overridden at runtime by View
            ...
            box.SetActive(false); // Hidden by default, View activates as needed
        }
    }
