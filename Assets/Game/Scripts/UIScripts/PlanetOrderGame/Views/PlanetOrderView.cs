...
/// <summary>
/// View for PlanetOrder game — split screen, each player has 3 boxes with numbers.
/// Player taps boxes in ascending order.
/// </summary>
public class PlanetOrderView : MonoBehaviour
{
    [Header("=== Common UI ===")]
    ...
    [SerializeField] private Sprite[] planetSprites;  // 10 planet images (random bg per box)
    ...
    // Events: playerIndex (0/1), boxIndex (0/1/2)
    ...
    /// <summary>
    /// Display N numbers for a player. Shows only the needed boxes, hides the rest.
    /// </summary>
    public void ShowBoxes(int playerIndex, int[] values)
    {
        Button[] btns = GetButtons(playerIndex);
    ...
    private Vector2[] GenerateRandomPositions(int count, float xMin, float xMax, float yMin, float yMax, float[] boxWs, float[] boxHs)
    {
        ...
        // Predefined zone layouts by count — pick one randomly
        float[][,] layouts3 = new float[][,]
        {
            ...
            new float[,] { {0.85f, 0.25f}, {0.50f, 0.70f}, {0.15f, 0.30f} },
        };
        float[][,] layouts4 = new float[][,]
        {
            ...
            new float[,] { {0.25f, 0.20f}, {0.75f, 0.40f}, {0.25f, 0.70f}, {0.75f, 0.80f} },
        };
        float[][,] layouts5 = new float[][,]
        {
            ...
            new float[,] { {0.25f, 0.18f}, {0.75f, 0.18f}, {0.15f, 0.55f}, {0.85f, 0.55f}, {0.50f, 0.82f} },
        };
