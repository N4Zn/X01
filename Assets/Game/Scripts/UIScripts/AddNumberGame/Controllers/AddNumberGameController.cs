...
/// <summary>
/// Controller for the AddNumber game scene.
/// Split-screen 2-player math game (1024x600).
/// New gameplay: equation A + B = C with hidden operand, pick 1 of 4 answers.
/// </summary>
public class AddNumberGameController : MonoBehaviour
{
    [SerializeField] private AddNumberGameView gameView;
    ...
    private AddNumberGameModel _gameModel;
    ...
    void Update()
    {
        if (GetCurrentState() == AddNumberSceneState.Playing)
        {
            UpdateTimer();
        }
    }
    ...
    private void InitFSMManager()
    {
        ...
        _customFSMManager.Initialize(typeof(AddNumberSceneState), this.GetType(), false);
        _customFSMManager.StateMachineChange(AddNumberSceneState.Initialize);
    }

    public AddNumberSceneState GetCurrentState()
    {
        if (_customFSMManager == null) return AddNumberSceneState.Initialize;
        Enum state = _customFSMManager.GetCurrentState();
        if (state == null) return AddNumberSceneState.Initialize;
        return (AddNumberSceneState)state;
    }
    ...
    {
        Debug.Log("NDL: AddNumber - StateMachineEnter_Initialize");
        _customFSMManager.StateMachineChange(AddNumberSceneState.Tutorial);
    }

    protected void StateMachineExit_Initialize(Enum previousState, Dictionary<string, object> options)
    {
        Debug.Log("NDL: AddNumber - StateMachineExit_Initialize");
    }
    ...
    protected void StateMachineEnter_Tutorial(Enum previousState, Dictionary<string, object> options)
    {
        ...
        if (tutorialPanel != null)
        {
            ...
            if (tutorialClip != null)
                tutorialPanel.Show(tutorialClip, "Huong dan: AddNumber");
            else
                tutorialPanel.ShowPlaceholder("Huong dan: AddNumber");
        }
        else
        {
            StartNewGame();
    ...
    {
        Debug.Log("NDL: AddNumber - StateMachineExit_Tutorial");
        if (tutorialPanel != null)
            tutorialPanel.OnStartGame -= OnTutorialStartGame;
    }
    ...
    private void StartNewGame()
    {
        ...
        PlayerRecognitionService.Instance.BeginGameSession(GameSessionManager.ResolveActiveGameName("AddNumberGame"));
    ...
    private void UpdateTimer()
    {
        ...
        if (_gameModel.GameTimer <= 0)
        {
            ...
            _customFSMManager.StateMachineChange(AddNumberSceneState.GameOver);
        }
    }
    ...
    {
        if (GetCurrentState() != AddNumberSceneState.Playing) return;
    ...
    {
        Debug.Log("NDL: AddNumberGame - OnSettingClicked");
    }
}
